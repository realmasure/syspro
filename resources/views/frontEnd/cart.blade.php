...
@section('content')
        ...
        <div class="container">
            ...
            <div class="table-responsive cart_info">
                <table class="table table-condensed">
                    <thead>
                    <tr class="cart_menu">
                        <td class="image">আইটেম</td>
                        <td class="description"></td>
                        <td class="price">দাম</td>
                        <td class="quantity">সংখ্যা</td>
                        <td class="total">মোট</td>
                        <td></td>
                    </tr>
                    </thead>
                    <tbody>
    ...
    <section id="do_action">
        <div class="container">
            <div class="heading">
                <h3>পরবর্তী কাজ করুন</h3>
                <p>ডিসকাউন্ট বা ছাড় থাকলে কুপন ব্যবহার করুন</p>
            </div>
            <div class="row">
                <div class="col-sm-6">
                    ...
                    <div class="chose_area" style="padding: 20px;">
                        <form action="{{url('/apply-coupon')}}" method="post" role="form">
                            ...
                            <div class="form-group">
                                <label for="coupon_code">কুপন কোড</label>
                                <div class="controls {{$errors->has('coupon_code')?'has-error':''}}">
                                    <input type="text" class="form-control" name="coupon_code" id="coupon_code" placeholder="কুপন দিয়ে ছাড়">
                                    <span class="text-danger">{{$errors->first('coupon_code')}}</span>
                                </div>
                                <button type="submit" class="btn btn-primary">প্রয়োগডরুন</button>
                            </div>
                        </form>
                    </div>
                </div>
                    ...
                    <div class="total_area">
                        <ul>
                            @if(Session::has('discount_amount_price'))
                                <li>মোট <span>$ {{$total_price}}</span></li>
                                <li>কুপন ডিসকাউন্ট (কোড : {{Session::get('coupon_code')}}) <span>‎৳ {{Session::get('discount_amount_price')}}</span></li>
                                <li>মোট <span>‎৳ {{$total_price-Session::get('discount_amount_price')}}</span></li>
                            @else
                                <li>মোট <span>‎৳ {{$total_price}}</span></li>
                            @endif
                        </ul>
                        <div style="margin-left: 20px;"><a class="btn btn-default check_out" href="{{url('/check-out')}}">চেক আউট</a></div>
                    </div>
                </div>
            </div>
        </div>
